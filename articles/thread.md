...
在线程池的内置api中，除了`execute`方法可以执行任务外，还可以使用`submit`实现，但是两者是存在区别的，前者是无返回值的，而后者是会返回`Future<T>`，那么`Future`是什么，又是如何工作的呢？在了解`Future`之前，我们需要先了解`Callable`是什么。
...
> 相比`Runnable #run`方法，`Callable #call`可以`返回结果或抛出异常`，但是`只能够在线程池中`使用，并不能在`Thread`中使用。
>
> 那么结合线程池的submit方法，是如何通过Future获取返回值的呢？咱们继续往下看。
