...
Java虚拟机将`常量池内的符号引用替换为直接引用`的过程。《Java虚拟机规范》没有规定`解析`阶段发生的具体时间，只要求了执行`ldc、getfield、getstatic等17个指令前`，先对它们所使用的`符号引用`进行解析。所以虚拟机可以自行决定是在**类被加载器加载时**或**等到符号引用将被使用前**再对符号引用进行解析。
...
>
> 2. 若A是数组类型(如Integer[])，那么虚拟机会将`x[Ljava/lang/Integer`中的`Integer`类型交给`Test类加载器`去加载，再由虚拟机生成对应的`数组对象`。
>
...
2. 否则，查找A中是否存在`简单名称`和`字段描述`都与目标匹配的方法，返回该方法的直接引用。
